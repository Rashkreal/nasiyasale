...
import { List, RefreshCw, CheckCircle, Copy, Check, XCircle, Lock, AlertCircle, Search, Filter } from 'lucide-react';

function shortAddr(a) { return !a || a === ethers.ZeroAddress ? '—' : a.slice(0,6)+'...'+a.slice(-4); }
function durFmt(r) { try { return parseFloat(ethers.formatUnits(r,18)).toLocaleString(undefined,{maximumFractionDigits:4}); } catch { return '?'; } }
function usdtFmt(r) { try { return parseFloat(ethers.formatUnits(r,6)).toLocaleString(undefined,{maximumFractionDigits:2}); } catch { return '?'; } }
...
function AddrCell({ addr }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'4px' }}>
      <span className="address" title={addr}>{shortAddr(addr)}</span>
...
function getStatusLabel(status, t) {
  const map = {
    0: t('statusPendingSellerC'), 1: t('statusPendingBuyerC'),
    2: t('statusPendingSellerN'), 3: t('statusPendingBuyerN'),
    4: t('statusApproved'), 5: t('statusRemoved'),
    6: t('statusPaid'), 7: t('statusClaimCol'), 8: t('statusClaimBL')
  };
...
export default function Listings() {
  ...
  const [filterType, setFilterType] = useState('all'); // all, collateral, nocollateral

  // id => chosen token key (faqat PendingSellerCollateral uchun xaridor tanlaydi)
  const [chosenTokens, setChosenTokens] = useState({});
  // id => { tokenKey: formattedAmount }  — locked narx asosida
  ...
  const fetchListings = useCallback(async () => {
    ...
    } catch(e) { console.error(e); toast.error(t('errorOccurred')); }
    finally { setLoading(false); }
  }, [contract, readOnlyContract, t]);

  useEffect(() => { fetchListings(); }, [fetchListings]);

  // ✅ E'lon paytida lock qilingan narx asosida garov miqdorini olish
  const fetchCollateralPreview = useCallback(async (listingId, tokenKey) => {
    const c = contract || readOnlyContract;
    if (!c) return;
    setPreviewLoading(prev => ({ ...prev, [`${listingId}_${tokenKey}`]: true }));
    try {
      const tokenId = TOKEN_IDS[tokenKey];
      // getCollateralAmount → lockedPrices ishlatadi (e'lon paytidagi narx)
      ...
      setCollateralPreviews(prev => ({
        ...prev,
        [listingId]: { ...prev[listingId], [tokenKey]: formatted }
      }));
    } catch (e) {
      console.error('preview err:', e);
    } finally {
      setPreviewLoading(prev => ({ ...prev, [`${listingId}_${tokenKey}`]: false }));
    }
  }, [contract, readOnlyContract]);

  // PendingSellerCollateral e'lonlari yuklanganda — barcha tokenlar uchun preview bir marta yuklash
  ...
  const handleChooseToken = (listingId, tokenKey) => {
    setChosenTokens(prev => ({ ...prev, [listingId]: tokenKey }));
    // Preview allaqachon bor, lekin yo'q bo'lsa qayta olish
  ...
  const handleApprove = async (listingId, listing) => {
    if (!account) { toast.error(t('connectPrompt')); return; }
    if (!account || !contract || !signer) return toast.error(t('connectPrompt'));
    const status = Number(listing.status);
    let chosenTokenId = 0;

    if (listing.isCollateral && status === 0) {
      // PendingSellerCollateral — xaridor token tanlaydi va garov beradi
      const chosenKey = chosenTokens[listingId];
      if (!chosenKey) return toast.error('Garov tokenini tanlang!');
      chosenTokenId = TOKEN_IDS[chosenKey];
      // ✅ lockedPrices asosida hisoblangan miqdor — flash loan xavfi yo'q
      const colAmt = await contract.getCollateralAmount(listingId, chosenTokenId);
      await ensureApproval(chosenKey, colAmt);
    }

    // PendingBuyerCollateral (1) yoki PendingBuyerNoCollateral (3) — sotuvchi DUR beradi
    ...
    try {
      ...
      if (msg.includes('user rejected')) userMsg = t('walletRejected');
      else if (msg.includes('insufficient') || msg.includes('exceeds')) userMsg = "Hamyonda yetarli token yo'q!";
      else if (msg.includes('BL low') || msg.includes('buyer BL')) userMsg = t('listingsLowBL');
  ...
  const handleCancel = async (listingId, listing) => {
    ...
    try {
      ...
      const msg = e?.reason || e?.message || '';
      toast.error(msg.includes('user rejected') ? t('walletRejected') : t('errorOccurred'), { id: toastId });
    } finally { setActionLoading(null); }
  };

  // account yo'q bo'lsa ham e'lonlar ko'rinadi, faqat tranzaksiya tugmalari yashiriladi
  ...
  const filteredList = currentList.filter(listing => {
    ...
    // Qidiruv — ID, manzil, DUR/USDC miqdori
    if (searchQuery.trim()) {
      ...
      if (!id.includes(q) && !seller.includes(q) && !buyer.includes(q) && !dur.includes(q) && !usdc.includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div>
      <div className="page-header" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        ...
        <button className="btn btn-outline btn-sm" onClick={fetchListings} disabled={loading}>
      ...
      <div style={{ display:'flex', gap:'8px', marginBottom:'12px' }}>
        {[
          ...
          <button key={key} onClick={() => setTab(key)}
            ...
            {label}
            <span style={{ background: tab === key ? 'rgba(255,255,255,0.2)' : 'var(--border)', padding:'0 7px', borderRadius:'10px', fontSize:'12px' }}>{count}</span>
          </button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:'8px', marginBottom:'16px', flexWrap:'wrap' }}>
        <div style={{ position:'relative', flex:1, minWidth:'180px' }}>
          ...
          <input className="input" placeholder={t('searchPlaceholder') || "ID, manzil yoki miqdor..."}
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign:'center', padding:'48px' }}>
          <List size={32} color="var(--text-muted)" style={{ marginBottom:12 }} />
          <p style={{ color:'var(--text-secondary)' }}>{t('listingsEmpty')}</p>
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
          {filteredList.map(listing => {
            ...
            // PendingBuyerCollateral — garov contractda saqlangan
            ...
            const lockedColAmt = isPendingBuyerCollateral && listing.collateralAmount > 0
              ? parseFloat(ethers.formatUnits(listing.collateralAmount, TOKEN_DECIMALS[lockedTokenName] || 18)).toFixed(
                  (TOKEN_DECIMALS[lockedTokenName] || 18) <= 8 ? 6 : 4
                )
              : null;

            // PendingSellerCollateral — xaridor token tanlaydi
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:'8px', flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
                      ...
                      <span className="badge" style={{
                        ...
                        {listing.isCollateral ? `🔒 ${t('listingsCollateral')}` : `🤝 ${t('listingsNoCollateral')}`}
                      </span>
                      <span className="badge badge-warning">{getStatusLabel(status, t)}</span>
                    </div>
                    ...
                    <div style={{ display:'flex', gap:'20px', flexWrap:'wrap' }}>
                      ...
                      <div><div style={{ fontSize:'11px', color:'var(--text-muted)' }}>{t('listingsColSeller')}</div><AddrCell addr={listing.seller} /></div>
                      <div><div style={{ fontSize:'11px', color:'var(--text-muted)' }}>{t('listingsColBuyer')}</div><AddrCell addr={listing.buyer} /></div>
                      <div><div style={{ fontSize:'11px', color:'var(--text-muted)' }}>{t('listingsColPeriod')}</div><div style={{ fontSize:'13px' }}>{Number(listing.paymentPeriod)} {t('days')}</div></div>
                    </div>

                    {/* ✅ PendingSellerCollateral: har bir token uchun locked narx asosida garov miqdori */}
                    {isPendingSellerCollateral && acceptedTokens.length > 0 && (
                      <div style={{ marginTop:'4px' }}>
                          ...
                          {acceptedTokens.map(tk => {
                            ...
                            return (
                              <div key={tk} style={{
                                ...
                                <span style={{ fontWeight:700, color }}>{tk}</span>
                                <span style={{ color:'var(--text-primary)', fontFamily:'Space Mono, monospace', fontWeight:600 }}>
                                  {isLoading ? '...' : amt ? amt : '—'}
                                </span>
                              </div>
                            );
                        ...
                        <div style={{ fontSize:'10px', color:'var(--text-muted)', marginTop:'6px', display:'flex', alignItems:'center', gap:'4px' }}>
                          <AlertCircle size={10} />
                    ...
                    {isPendingBuyerCollateral && lockedTokenName && (
                      <div style={{ display:'flex', alignItems:'center', gap:'6px', marginTop:'4px' }}>
                        ...
                        <span style={{ fontSize:'11px', color:'var(--text-muted)' }}>Contractdagi garov:</span>
                        <span style={{
                          ...
                          {lockedColAmt ? `${lockedColAmt} ` : ''}{lockedTokenName}
                        </span>
                      </div>
                    )}
                  </div>

                  {/* O'ng tomon: token tanlash + tasdiqlash tugmasi */}
                  <div style={{ display:'flex', flexDirection:'column', gap:'8px', alignItems:'flex-end' }}>

                    {/* PendingSellerCollateral — xaridor token tanlaydi */}
                    {canApprove && isPendingSellerCollateral && (
                      <div style={{ display:'flex', flexDirection:'column', gap:'6px', alignItems:'flex-end' }}>
                        <div style={{ fontSize:'11px', color:'var(--text-muted)' }}>Garov tokenini tanlang:</div>
                        <div style={{ display:'flex', gap:'4px', flexWrap:'wrap', justifyContent:'flex-end' }}>
                          {acceptedTokens.map(tk => {
                            ...
                            const color = TOKEN_COLORS[tk];
                            return (
                              <button key={tk} onClick={() => handleChooseToken(id, tk)}
                                style={{
                        ...
                        {/* Tanlangan token uchun batafsil ma'lumot */}
                        {chosenKey && collateralPreviews[id]?.[chosenKey] && (
                          <div style={{
                            fontSize:'12px', textAlign:'right', padding:'6px 10px',
                            borderRadius:'7px',
                            background: `${TOKEN_COLORS[chosenKey]}15`,
                            border: `1px solid ${TOKEN_COLORS[chosenKey]}40`,
                          }}>
                            <div style={{ color: TOKEN_COLORS[chosenKey], fontWeight:700, fontFamily:'Space Mono, monospace' }}>
                    ...
                    {/* PendingBuyerCollateral — sotuvchi uchun: garov contractda */}
                    {canApprove && isPendingBuyerCollateral && (
                      <div style={{ fontSize:'11px', color:'var(--success)', textAlign:'right', maxWidth:'150px' }}>
                    ...
                    {canApprove && (
                      <button className="btn btn-success btn-sm"
                        ...
                        {t('listingsApprove')}
                        {isPendingSellerCollateral && !chosenTokens[id] && (
                          <span style={{ fontSize:'10px', opacity:0.7 }}>(token tanlang)</span>
                        )}
                      </button>
                    )}
