...
// ─── Konstantalar ─────────────────────────────────────────────────────────────
const VAULT_ADDRESS = "0x0b76D70ae464f40046428B2F038A83fA01DEcd81";
...
function DepositForm({ signer, provider, account, onSuccess }) {
  ...
  const handleApprove = async () => {
    if (!signer) return toast.error("Hamyon ulanmagan");
    setApproving(true);
    try {
      const erc20 = new ethers.Contract(selectedToken.address, ERC20_ABI, signer);
      toast.loading("Approve...", { id: "approve" });
      const tx = await erc20.approve(VAULT_ADDRESS, rawAmount);
      await tx.wait();
      toast.success("Approve muvaffaqiyatli!", { id: "approve" });
      await fetchBal();
    } catch (err) {
  ...
  const handleDeposit = async () => {
    if (!signer) return toast.error("Hamyon ulanmagan");
    if (rawAmount === 0n) return toast.error("Miqdor kiriting");
    if (unlockTs === 0) return toast.error("Kun soni kiriting");
    if (!hasBalance) return toast.error("Balans yetarli emas");
    setDepositing(true);
    try {
      const vault = new ethers.Contract(VAULT_ADDRESS, VAULT_ABI, signer);
      toast.loading("Lock qilinmoqda...", { id: "deposit" });
      const tx = await vault.deposit(selectedToken.address, rawAmount, unlockTs);
      await tx.wait();
      toast.success(`${selectedToken.symbol} ${lockDays} kunga lock qilindi!`, { id: "deposit" });
  ...
  return (
    <div>
      ...
      <div className="form-group">
        ...
        <input className="form-input" type="text" placeholder="Masalan: 80000000"
          value={amount} onChange={e => setAmount(e.target.value)} />
        <div className="form-hint">
          Hamyon: {formatAmt(walletBalance, selectedToken.decimals)} {selectedToken.symbol}
          {walletBalance > 0n && (
      ...
      {rawAmount > 0n && !hasBalance && (
        <div style={{ padding: "10px 14px", background: "var(--danger-dim)", borderRadius: 8,
          ...
          <AlertTriangle size={14} /> Balans yetarli emas
        </div>
      )}
...
export default function Vault() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error("Vault yuklanishda xato");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // ── Write vault ───────────────────────────────────────────────────────────
  const getWriteVault = () => {
    if (!signer) { toast.error("Hamyon ulanmagan"); return null; }
  ...
  const handleWithdraw = (lockId) => withTx(`w${lockId}`, async () => {
    ...
    toast.loading("Yechilmoqda...", { id: `w${lockId}` });
    await (await v.withdraw(lockId)).wait();
    toast.success("Muvaffaqiyatli!", { id: `w${lockId}` });
  });
  ...
  const handleEmergency = (lockId) => withTx(`e${lockId}`, async () => {
    ...
    toast.success("Muvaffaqiyatli!", { id: `e${lockId}` });
  });

  const handleWithdrawNFT = (id) => withTx(`wn${id}`, async () => {
    const v = getWriteVault(); if (!v) return;
    toast.loading("NFT yechilmoqda...", { id: `wn${id}` });
    await (await v.withdrawNFT(id)).wait();
    toast.success("NFT yechildi!", { id: `wn${id}` });
  });
  ...
  const handleEmergencyNFT = (id) => withTx(`en${id}`, async () => {
    ...
    toast.success("Muvaffaqiyatli!", { id: `en${id}` });
  });
  ...
  const handleDepositNFT = () => withTx("depositNFT", async () => {
    ...
    if (approved.toLowerCase() !== VAULT_ADDRESS.toLowerCase()) {
      toast.loading("NFT approve qilinmoqda...", { id: "depositNFT" });
      await (await posMgr.approve(VAULT_ADDRESS, depositNFTId)).wait();
    }

    const unlockTs = daysToUnlockTs(depositNFTDays);
    toast.loading("NFT lock qilinmoqda...", { id: "depositNFT" });
    await (await v.depositNFT(depositNFTId, unlockTs)).wait();
    toast.success(`NFT ${depositNFTDays} kunga lock qilindi!`, { id: "depositNFT" });
...
const countdown = (unlockTs, status) => {
  ...
  const renderStatistika = () => (
    <>
      <div className="stat-grid" style={{marginBottom:24}}>
        <div className="stat-card"><div className="stat-label">Faol locklar</div><div className="stat-value accent">{activeLocks.length}</div></div>
        <div className="stat-card"><div className="stat-label">Faol NFT locklar</div><div className="stat-value accent">{activeNFTLocks.length}</div></div>
        <div className="stat-card"><div className="stat-label">Tarix</div><div className="stat-value">{historyLocks.length + historyNFTLocks.length}</div></div>
      ...
      <div className="card" style={{borderColor:"rgba(0,212,170,0.15)"}}>
        <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
          ...
          <p style={{fontSize:13,color:"var(--text-secondary)",lineHeight:1.6}}>
            DUR va BLT tokenlarning <strong style={{color:"var(--text-primary)"}}>90%</strong> i
            ushbu kontraktda qulflangan. Bu malumotlar onchain tekshirilishi mumkin.
            {withdrawalAddr && (
              <> Yechilgan tokenlar{" "}
                <strong style={{color:"var(--accent)",fontFamily:"var(--font-mono)",fontSize:11}}>
                  {withdrawalAddr.slice(0,8)}...{withdrawalAddr.slice(-6)}
                </strong> manziliga boradi.
              </>
            )}
          </p>
        </div>
  ...
  const renderLocklar = () => (
    <>
      <div className="page-header" style={{marginBottom:16}}>
        <h2 style={{fontSize:16,fontWeight:700}}>
          ...
          <span style={{marginLeft:8,fontSize:12,color:"var(--text-muted)",fontFamily:"var(--font-mono)",fontWeight:400}}>
            {filteredLocks.length} ta
          </span>
        </h2>
        <div className="page-actions">
          <div style={{position:"relative"}}>
            <Search size={13} style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"var(--text-muted)"}} />
            <input type="text" className="form-input" placeholder="Qidirish..."
      ...
      {filteredLocks.length === 0 ? (
        <div className="empty-state"><Lock size={40} /><p>Lock topilmadi</p></div>
      ) : (
        <div className="lock-list">
          {filteredLocks
            ...
            const isEmergencyReady = ti ? ti.isEmergencyReady : false;
            const cd = countdown(lock.unlockTimestamp, st);
            const statusLabel = st === 0 ? (isUnlocked ? "Ochilgan" : "Faol") : st === 1 ? "Yechildi" : "Emergency";
            ...
            return (
              <div key={lock.id} className={"lock-row" + (isActive && isUnlocked ? " unlocked pulse-glow" : "")}>
                ...
                <div className="lock-time">
                  <div style={{fontSize:11,color:"var(--text-muted)",marginBottom:2}}>Unlock sanasi</div>
                ...
                <div>
                  ...
                  {isActive && isUnlocked && !isEmergencyReady && ti && (
                    <div style={{fontSize:10,color:"var(--warning)",marginTop:2}}>
                      Favqulodda: {emergencyCountdown(ti.secondsToEmergency)}
                    </div>
                  )}
                  {isActive && isEmergencyReady && (
                    <div style={{fontSize:10,color:"#ef4444",marginTop:2}}>Favqulodda tayyor</div>
                  )}
                </div>
                <div>
  ...
  const renderNFT = () => (
    <>
      <h2 style={{fontSize:16,fontWeight:700,marginBottom:16}}>
        Faol NFT locklar
        <span style={{marginLeft:8,fontSize:12,color:"var(--text-muted)",fontFamily:"var(--font-mono)",fontWeight:400}}>
          {activeNFTLocks.length} ta
        </span>
      </h2>
      {activeNFTLocks.length === 0 ? (
        <div className="empty-state"><Lock size={40} /><p>Faol NFT lock yoq</p></div>
      ) : (
        <div className="lock-list">
          {activeNFTLocks.map(nft => {
            ...
            const isUnlocked = ti ? ti.isUnlocked : false;
            const isEmergencyReady = ti ? ti.isEmergencyReady : false;
            const statusLabel = st === 0 ? (isUnlocked ? "Ochilgan" : "Faol") : st === 1 ? "Yechildi" : st === 2 ? "Emergency" : "Rebalanced";
            const statusClass = st === 0 ? (isUnlocked ? "unlocked" : "active") : st === 1 ? "withdrawn" : "emergency";
            return (
                ...
                <div className="lock-actions">
                  {st === 0 && isOwner && (
                    <button className="btn btn-ghost btn-sm" onClick={() => {
                      ...
                      toast.success(`Lock #${nft.id} tanlandi. Yangi NFT Token ID kiriting.`);
                    }}>
  ...
  const renderKiritish = () => (
    <div style={{maxWidth:520}}>
      <h2 style={{fontSize:16,fontWeight:700,marginBottom:20}}>Token lock qilish</h2>
      <div className="card" style={{marginBottom:24}}>
        <DepositForm signer={signer} provider={provider} account={account} onSuccess={load} />
      </div>
      <h2 style={{fontSize:16,fontWeight:700,marginBottom:16}}>NFT lock qilish</h2>
      <div className="card">
        <div className="form-group">
  ...
  const renderNarxlar = () => (
    <div style={{maxWidth:600}}>
      <h2 style={{fontSize:16,fontWeight:700,marginBottom:20}}>Narxlar va boshqaruv</h2>
      <div className="card" style={{marginBottom:16}}>
        <div className="card-title" style={{marginBottom:16}}>Joriy narxlar</div>
      ...
      <div className="card" style={{marginBottom:16}}>
        <div className="card-title" style={{marginBottom:16}}>Balans sinxronlash</div>
      ...
      <div className="card">
        <div className="card-title" style={{marginBottom:16}}>NFT Rebalance</div>
        <div className="form-group">
          <label className="form-label">Eski NFT lock ID (kontraktdagi #raqam)</label>
          ...
          <div className="form-hint">NFT tab dagi lock #raqami (masalan: #0, #1, #2)</div>
        </div>
        <div className="form-group">
          <label className="form-label">Yangi NFT Token ID (Uniswap position ID)</label>
    ...
    return (
      <>
        ...
        <h2 style={{fontSize:16,fontWeight:700,marginBottom:16}}>
          Token lock tarixi
          <span style={{marginLeft:8,fontSize:12,color:"var(--text-muted)",fontFamily:"var(--font-mono)",fontWeight:400}}>
            {historyLocks.length} ta
          </span>
        </h2>
        {historyLocks.length === 0 ? (
          <div className="empty-state" style={{padding:32}}><p style={{color:"var(--text-muted)",fontSize:13}}>Hali tarix yoq</p></div>
        ) : (
            ...
            {historyLocks.map(lock => {
              ...
              return (
                <div key={lock.id} className="lock-row" style={{opacity:0.75}}>
                  ...
                  <div className="lock-time">
                    <div style={{fontSize:11,color:"var(--text-muted)",marginBottom:2}}>Kiritilgan</div>
    ...
    return (
      <>
        ...
        <div className="animate-in vault-wrap">
          ...
          <div className="stat-grid" style={{marginBottom:24}}>
            <div className="stat-card">
              <div className="stat-label">Faol locklar</div>
              <div className="stat-value accent">{activePub.length}</div>
            </div>
            <div className="stat-card">
              <div className="stat-label">NFT locklar</div>
          ...
          <div className="card" style={{borderColor:"rgba(0,212,170,0.15)",marginBottom:24}}>
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              ...
              <p style={{fontSize:13,color:"var(--text-secondary)",lineHeight:1.6}}>
                DUR va BLT tokenlarning <strong style={{color:"var(--text-primary)"}}>90%</strong> i
                ushbu kontraktda qulflangan — 80M ta 10+ yilga, 10M ta 3 yilga.
                Bu malumotlar onchain tekshirilishi mumkin. Manipulyatsiya texnik jihatdan imkonsiz.
                {withdrawalAddr && (
                  <> Yechilgan tokenlar{" "}
                    <strong style={{color:"var(--accent)",fontFamily:"var(--font-mono)",fontSize:11}}>
                      {withdrawalAddr.slice(0,8)}...{withdrawalAddr.slice(-6)}
  ...
  // ── Owner view ─────────────────────────────────────────────────────────────
  const TABS = [
    { id: "statistika", label: "Statistika", icon: <Shield size={15} /> },
    { id: "locklar",    label: "Locklar",    icon: <Lock size={15} /> },
    { id: "nft",        label: "NFT",        icon: <Zap size={15} /> },
    { id: "tarix",      label: "Tarix",      icon: <Search size={15} /> },
    { id: "kiritish",   label: "Kiritish",   icon: <ArrowDownToLine size={15} /> },
    { id: "narxlar",    label: "Boshqaruv",  icon: <RefreshCw size={15} /> },
  ];
      ...
      <div className="animate-in vault-wrap">
        ...
        <div className="vault-layout">
          <div className="vault-sidebar">
            {TABS.map(tab => (
              <button key={tab.id}
                className={"vault-nav-item" + (activeTab === tab.id ? " active" : "")}
